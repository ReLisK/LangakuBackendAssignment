...
class TestUserDueCardsEndpoint:
    ...
    def test_due_cards_endpoint_with_bad_user_pk(self):
        ...
        self.url = reverse(
            "user-due-cards",
            kwargs={"pk": User.objects.count() + 1},
            query={"until": "2025-12-15"},
        )
        response = self.client.get(self.url)
    ...
    def test_due_cards_endpoint_with_no_until_parameter(self):
        ...
        self.url = reverse("user-due-cards", kwargs={"pk": self.user.id})
        response = self.client.get(self.url)
    ...
    def test_due_cards_endpoint_with_bad_date_format(self):
        ...
        self.url = reverse(
            "user-due-cards", kwargs={"pk": self.user.id}, query={"until": "12-2025-15"}
        )
        response = self.client.get(self.url)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestReviewsEndpoint:
    def setup_method(self):
        ...
        self.test_username = "testuser"
        self.user = User.objects.create_user(username=self.test_username)
        self.testCard = Card.objects.create(
            user=self.user, front="TestCardFront", back="TestCardBack", step=0
        )
        self.data = {"card": self.testCard.id, "ratingScore": 2}

    def test_Reviews_endpoint_Idempotency(self):
        """Test that duplicate requests are blocked"""
        response = self.client.post(
            self.url, data=self.data, HTTP_X_IDEMPOTENCY_KEY=1234
        )
        response = self.client.post(
            self.url, data=self.data, HTTP_X_IDEMPOTENCY_KEY=1234
        )
        ...
        assert response.data["detail"] == "Duplicate request denied"

    def test_Reviews_endpoint_success_response(self):
        """Test that review endpoint responds with success (201)"""
        response = self.client.post(
            self.url, data=self.data, HTTP_X_IDEMPOTENCY_KEY=1234
        )

        assert response.status_code == status.HTTP_201_CREATED
        assert "next_review" in response.data
